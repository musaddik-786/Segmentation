...
def extract_fnol_fields_from_text(req: ExtractFnolFieldsRequest):
    ...
    try:
        ...
        # occupancy_at_loss comes back as bool/string from the LLM — normalise
        # to "Yes"/"No" text so it's stored and displayed consistently (not
        # 1/0), matching the DB column's TEXT type.
        occ_info = fields.get("occupancy_at_loss", {})
        occ_val = occ_info.get("value")
        if occ_val is not None:
            if isinstance(occ_val, bool):
                update_data["occupancy_at_loss"] = "Yes" if occ_val else "No"
            elif isinstance(occ_val, str):
                update_data["occupancy_at_loss"] = (
                    "Yes" if occ_val.strip().lower() in ("true", "yes", "1") else "No"
                )
        ...
        # overall_confidence is no longer taken from this turn's raw extraction
        # score — update_fnol_submission (called above via update_data, when
        # there was anything new to persist) recomputes it itself from the
        # cumulative record every time a field changes. A single turn's score
        # only reflects that turn's isolated message and collapses toward 0 on
        # short later replies (e.g. "yes") even when the record overall is
        # well-populated, so it's discarded here rather than persisted.

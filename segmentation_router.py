...
def is_pdf_header_footer(line: str) -> bool:
    ...
    # Document notice
    if line.startswith("DOCUMENT NOTICE"):
        return True

    # Continuation of document notice
    if line == "contract.":
        return True
...
def chunk_documents(documents: list[dict]):
    ...
    for document in documents:
        ...
        for line in lines:
            ...
            # CLAUSE
            # -----------------------------------------
            # Do not treat Schedule A values such as
            # 30.0, 70.0, 27.5, etc. as clauses.

            if not (
                current_article
                and current_article.upper().startswith("SCHEDULE")
            ):

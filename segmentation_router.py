this is mky start services.psh
#!/bin/bash
# start_services.sh
# Lives on the VM at:
...
and below are the results from terminal


azureuser@slm-ft-t4:~/Ramakrishna/claims-SLM-Finetune$ grep PYTHON /home/azureuser/Ramakrishna/claims-SLM-Finetune/start_services.sh && bash /home/azureuser/Ramakrishna/claims-SLM-Finetune/start_services.sh
PYTHON=$BASE/../slm-env/bin/python
nohup $PYTHON MCP/main.py > "$LOG_DIR/mcp.log" 2>&1 &
nohup $PYTHON MotorIntakeValidationAgent/server.py > "$LOG_DIR/intake.log" 2>&1 &
nohup $PYTHON MotorTriageAgent/server.py > "$LOG_DIR/triage.log" 2>&1 &
...
[cleanup] Stopped existing process on port 5000 (PID 4813)
[1/4] MCP server started (PID 5015, port 8500) — waiting 10s...
[2/4] Intake Validation Agent started (PID 5041, port 8501) — waiting 10s...
[3/4] Motor Triage Agent started (PID 5061, port 8502) — waiting 20s for model load...
[4/4] Frontend app started (PID 5081, port 5000) — waiting 8s...

=== Port check ===
  [!!]  Port 8500 is NOT listening — check /tmp/motor_demo_logs/*.log
  [!!]  Port 8501 is NOT listening — check /tmp/motor_demo_logs/*.log
  [!!]  Port 8502 is NOT listening — check /tmp/motor_demo_logs/*.log
  [OK]  Port 5000 is listening

Logs: /tmp/motor_demo_logs/
Done. Port forwarding will connect now.
azureuser@slm-ft-t4:~/Ramakrishna/claims-SLM-Finetune$ cat /tmp/motor_demo_logs/mcp.log
nohup: failed to run command '/home/azureuser/Ramakrishna/claims-SLM-Finetune/../slm-env/bin/python': No such file or directory

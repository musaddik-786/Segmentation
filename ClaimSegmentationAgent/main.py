...
Registered sub-apps:
  ...
  /api/v1/slm                       — SLM Triage (Mock)                  [MOCK]
...
segmentation_app = _make_cors_app(
    ...
    description="MCP tools for STP scoring and claim segmentation (Homeowners and Motor LOB).",
)
...
FastApiMCP(
    ...
    include_operations=[
        ...
        "get_claim_documents",
        "get_document",
        "reassign_documents_claim_number",
    ],
).mount_http()
...
policy_coverage_app = _make_cors_app(
    title="policy_coverage_agent_mcps",
    description="MCP tools for policy lookup, coverage verification, and payment tracking (Homeowners: Guidewire PC, Motor: local dummy tables).",
)
policy_coverage_app.include_router(policy_coverage_router)
FastApiMCP(
    policy_coverage_app,
...
# ── Sub-app: slm (SLM Triage — MOCK) ─────────────────────────────────────────

slm_app = _make_cors_app(
    title="slm_agent_mcps",
    description="MCP tools for mock SLM triage: fetches documents submitted by the "
                "Document Submission Agent and returns a randomly-selected triage verdict "
                "from the 12 standard insurance triage categories.",
)
slm_app.include_router(slm_router)
FastApiMCP(
    slm_app,
    include_operations=[
        "triage_claim",
